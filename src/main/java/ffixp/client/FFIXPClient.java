...
import ffixp.client.ui.FFIXPFrame;
 ...
 */
public class FFIXPClient {
    private static final CountDownLatch shutdownLatch = new CountDownLatch(1);

    private static final Logger log = LoggerFactory.getLogger(FFIXPClient.class);
    private static FFIXPClient banzai;
    private boolean initiatorStarted = false;
    private Initiator initiator = null;
    private JFrame frame = null;

    public FFIXPClient(String[] args) throws Exception {
        InputStream inputStream = null;
        if (args.length == 0) {
            inputStream = FFIXPClient.class.getResourceAsStream("banzai.cfg");
        } else if (args.length == 1) {
            inputStream = new FileInputStream(args[0]);
        }
        if (inputStream == null) {
            System.out.println("usage: " + FFIXPClient.class.getName() + " [configFile].");
            return;
        }
